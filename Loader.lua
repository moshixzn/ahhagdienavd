...
-- thread handles — all loops are task.spawn threads now, cancellable
local Threads = {}
...
-- Searches all of RS recursively for a RemoteEvent/Function whose name
-- matches any of the supplied plain-text patterns (case-insensitive).
...
-- Tries every known Roblox interaction method on a given egg model.
-- ProximityPrompt → ClickDetector → local RemoteEvents → RS pattern scan.
local function tryInteractEgg(egg)
    -- ProximityPrompts (most common in modern pet/egg games)
    ...
    -- RemoteEvents attached directly to the egg model
    ...
    -- Broad RS scan — fire with multiple arg signatures since we don't
    -- know what the server handler expects
...
-- Kills a named thread if it exists
...
local function startStealLoop()
    ...
    Threads["steal"] = task.spawn(function()
        ...
        while State.AutoSteal do
            if not (State.StealOnce and stolen) then
                for _, egg in ipairs(findEggs()) do
                    ...
                    if rarityMatches(egg.Name, State.RarityFilter) then
                        ...
                        if part then
                            ...
                            tryInteractEgg(egg)
                            stolen = true
                            if State.StealOnce then break end
                        end
                    end
                    task.wait(0.1)
                end
    ...
    Threads["hatch"] = task.spawn(function()
        ...
        while State.AutoHatch do
            if not (State.HatchOnce and hatched) then
                -- Fire any RS hatch remote with multiple arg signatures
                local remote = findRemote("hatch", "open", "crack", "incubat")
                if remote then
                    pcall(function() remote:FireServer() end)
                    hatched = true
                end
                -- Also sweep workspace for hatch ProximityPrompts
                for _, obj in ipairs(workspace:GetDescendants()) do
    ...
    Threads["claim"] = task.spawn(function()
        while State.AutoClaim do
            ...
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj:IsA("ProximityPrompt") then
                    local txt = (obj.ActionText .. obj.ObjectText):lower()
                    if txt:find("claim", 1, true) or txt:find("reward", 1, true)
                    or txt:find("collect", 1, true) then
                        pcall(function()
                            if fireproximityprompt then fireproximityprompt(obj) end
                        end)
                    end
    ...
    Threads["treadmill"] = task.spawn(function()
        while State.AutoTreadmill do
            ...
            if treadmill then
                ...
                if part then
                    teleportTo(part.Position + Vector3.new(0, 3, 0))
                    -- Trigger all ProximityPrompts on the treadmill
...
HopTab:Toggle({
    ...
    Callback = function(v)
        State.AutoHop = v
        State.HopCount = 0  -- reset counter on each enable
        if v then
            killThread("hop")
...
HatchTab:Toggle({
    ...
    Desc  = "Shows egg locations through walls",
    Callback = function(v)
        State.EggESP = v
        if v then
            startESPLoop()
...
EquipTab:Toggle({
    ...
    Callback = function(v)
        ...
        if v then startSellEggLoop() end  -- was missing before
    end,
})
EquipTab:Space()
EquipTab:Dropdown({
...
SettingsTab:Button({
    ...
    Icon     = "solar:info-square-bold",
    Callback = function()
        -- kill all threads cleanly before destroying
